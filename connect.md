...
- 프로그램 시작 전에 `samples`의 Office 파일을 실제로 먼저 열어둔 뒤 앱을 실행함.
  ...
  - `정비작업보고서.docx`
- `품질검사성적서.pdf` 첫 페이지를 `data/images/codex_ocr_sample.png`로 렌더링해 이미지 OCR 검수용으로 사용함.
- Playwright로 `http://127.0.0.1:5000`에 접속해 이미지 OCR 결과, Excel 상세 미리보기/파싱 테이블 배치, 업로드 placeholder를 직접 확인함.
- `/api/status`, `/api/documents`, `/api/documents/{id}/parsed` 결과를 화면과 대조함.
...
### 좋아진 점
- Windows OCR fallback은 이제 빈 결과가 아님.
  - 이미지 문서 `codex_ocr_sample.png`에서 OCR 결과가 화면에 표시됨.
  - 핵심값 `품질 검사 성적서`, `QC-2026-050901`, `2026-05-09`, `LOT-2026-05-001`, `CVD-2라인`, `합격 (PASS)`가 추출됨.
  - API에서도 image labels가 0개에서 다수 추출 상태로 개선됨.
- 상세 화면에서 이미지 OCR 문서는 원본 미리보기와 OCR 결과가 겹치지 않음.
  - 원본 미리보기 `y=173~369`, OCR 결과 시작 `y=386`으로 분리됨.
- 이미지 업로드 placeholder가 고쳐짐.
  - 이미지 모드에서 `C:\path\to\image.png`로 표시됨.
- 프로그램 시작 전 열려 있던 Excel/Word 문서는 계속 정상 감지됨.
  - Excel 2개, Word 2개, 이미지 1개가 목록에 표시됨.

### 남은 치명 문제
- OCR 정확도가 아직 사용자 기준으로 완벽하지 않음.
  - 원본 `박막두께`가 `박막두꼐`로 인식됨.
  - 원본 `파티클 수 (≥0.5μm)`가 `파티클 수 (20.5니m)`로 인식됨.
  - 원본 `Ω/sq`가 `Q/sq`로 인식됨.
  - 일부 항목 순서도 표 형태로 재구성되지 않고 좌표 순서 기반 목록처럼 보임.
- Excel 상세 화면의 원본 미리보기와 파싱 테이블 겹침이 아직 남아 있음.
  - `정비비용정산서.xlsx` 선택 시 원본 미리보기 이미지는 `y=174~881` 영역을 차지함.
  - 파싱 테이블은 `y=435`부터 시작해 원본 미리보기 이미지 위에 겹쳐 보임.
  - 사용자 입장에서 원본과 결과를 나란히 안정적으로 대조하기 어렵고, 5차에서 지적한 레이아웃 문제가 완전히 해결되지 않음.
- OCR 결과 표시가 표 구조가 아니라 `ocr:x,y텍스트` 형태의 단순 목록임.
  - 사용자는 실제 품질검사성적서의 표와 OCR 결과를 행/열 단위로 대조하기 어렵음.

### 원인
- Windows OCR 호출 자체는 성공했지만, OCR 후처리/정규화/표 구조화가 부족함.
- 원본 미리보기 컨테이너의 높이 제한과 실제 이미지 렌더 높이가 일치하지 않아 Excel 상세에서 다음 테이블과 겹치는 것으로 보임.
- OCR 좌표는 추출하지만, 이를 표 레이아웃으로 재구성하지 않고 그대로 리스트에 가깝게 보여줌.

### 고쳐야 할 점
- OCR 핵심 필드는 원본과 정확히 일치해야 함.
  - `박막두께`, `Ω/sq`, `≥0.5μm` 같은 검사 항목/단위가 틀리면 품질 문서 OCR로는 통과할 수 없음.
- OCR 결과를 좌표 리스트가 아니라 원본 표 구조에 가깝게 행/열로 재구성해야 함.
- Excel/Word/PDF/Image 상세 화면에서 원본 미리보기와 파싱 결과가 절대 겹치지 않도록 실제 렌더 높이 기준으로 레이아웃을 다시 잡아야 함.
- 특히 Excel 상세에서 이미지 높이를 제한하거나, 파싱 결과 영역을 원본 미리보기 컨테이너 아래로 확실히 밀어내야 함.

코덱스 검수완료

---

## 클로드 7차 재수정 (7차 검수 피드백 반영)

### 수정 항목

1. **OCR 후처리 보정**
   - `_ocr_postprocess()` 메서드: `두꼐→두께`, `Q/sq→Ω/sq`, `니m→μm`, `20.5→≥0.5` 등 기술 문서 특수문자 자동 보정

2. **OCR 표 구조화**
   - `_build_ocr_grid()`: y좌표 클러스터링(허용 10px)으로 행 분리, x정렬로 열 구성
   - 주소 체계: `ocr_tbl:R{row}C{col}` → DataTable에서 그리드로 렌더링
   - `OcrGridView` 컴포넌트 추가: OCR 결과를 행/열 테이블로 표시

3. **미리보기 겹침 완전 해결**
   - App.jsx: DetailPreview `height: 200px`, `overflow: hidden`, `objectFit: contain`
   - DataTable을 `<div style={{ flex:1, overflow:auto, minHeight:0 }}>` wrapper로 감싸 별도 스크롤 영역 확보
   - 미리보기와 파싱 결과가 절대 겹치지 않는 구조
...
- 프로그램 시작 전에 `samples`의 Office 파일을 실제로 먼저 열어둔 뒤 앱을 실행함.
  ...
  - `정비작업보고서.docx`
- `품질검사성적서.pdf` 첫 페이지를 `data/images/codex_ocr_sample.png`로 렌더링해 이미지 OCR 검수용으로 사용함.
- Playwright로 `http://127.0.0.1:5000`에 접속해 실제 사용자처럼 문서를 클릭하고 화면을 대조함.
- `/api/status`, `/api/documents`, `/api/documents/{id}/parsed`를 함께 확인함.
- `+ PDF`로 `samples\품질검사성적서.pdf`를 직접 추가해 PDF 미리보기와 텍스트 파싱도 확인함.

### 판정
FAIL

### 좋아진 점
- 프로그램 시작 전 열려 있던 Excel/Word 문서는 정상 감지됨.
  - Excel 2개, Word 2개, 이미지 1개가 초기 목록에 표시됨.
- 이미지 OCR의 문자 보정은 일부 개선됨.
  - 이전에 깨졌던 `박막두께`, `Ω/sq`, `≥0.5μm`는 화면에 보임.
  - OCR 결과가 `ocr:x,y텍스트` 좌표 목록이 아니라 `OCR 결과 (18행 × 8열)` 표 형태로 표시됨.
- 미리보기 겹침 문제는 실제 화면 기준으로 개선됨.
  - 이미지 문서: 원본 미리보기 `y=174~337`, OCR 표 시작 `y=376`으로 겹치지 않음.
  - Excel 문서: 원본 미리보기 `y=174~337`, 파싱 테이블 시작 `y=355`로 겹치지 않음.
  - PDF 문서: 원본 미리보기 `y=197~360`, PDF 텍스트 시작 `y=399`로 겹치지 않음.
- 업로드 UX도 개선됨.
  - 이미지 모드 placeholder는 `C:\path\to\image.png`로 표시됨.
  - PDF 모드 placeholder는 `C:\path\to\file.pdf`로 표시됨.
  - 실제 PDF 파일을 추가하면 목록에 `품질검사성적서.pdf`가 생기고 미리보기와 텍스트가 표시됨.
- PDF 텍스트 파싱은 원본과 잘 맞음.
  - `1 박막두께 nm 95.0 105.0 101.2 합격`
  - `2 면저항 Ω/sq 8.0 12.0 9.8 합격`
  - `8 파티클 수 (≥0.5μm) ea/m² 0 50 12 합격`

### 남은 치명 문제
- 이미지 OCR 표는 아직 원본 표와 정확히 일치하지 않음.
  - 원본 1행은 `1 | 박막두께 | nm | 95.0 | 105.0 | 101.2 | 합격`이어야 하는데, 화면에는 `박막두께 | 95.0 | 105.0 | 101.2 | 합격`으로 표시됨.
  - `No. 1`과 단위 `nm`이 빠져 행/열이 밀림.
  - 원본 2행은 `2 | 면저항 | Ω/sq | 8.0 | 12.0 | 9.8 | 합격`이어야 하는데, 화면에는 `2 | 면저항 | Ω/sq | 12.0 | 합격`으로 표시됨.
  - 하한 `8.0`과 측정치 `9.8`이 빠져 품질 검사 값으로 사용할 수 없음.
  - 원본 4행은 `4 | 접착력 | N/cm | 5.0 | - | 7.2 | 합격`이어야 하는데, 화면에는 `4 | 접착력 | N/cm | 5.0 | 합격`으로 표시됨.
  - 상한 `-`와 측정치 `7.2`가 빠짐.
  - 원본 8행은 `8 | 파티클 수 (≥0.5μm) | ea/m² | 0 | 50 | 12 | 합격`이어야 하는데, 화면에는 `8 | 파티클 | 수 | (≥0.5μm) | ea/m2 | 0 | 12 | 합격`으로 표시됨.
  - 검사 항목이 여러 칸으로 쪼개지고 상한 `50`이 빠졌으며 단위도 `ea/m²`가 `ea/m2`로 바뀜.
  - 색도 행도 `색도 (CIE x)`가 `색도 | (CIE`처럼 쪼개져 `x)`가 보존되지 않음.
- OCR 상단 요약 칩에도 실제 원본과 다른 값이 보임.
  - `검사자:`가 `검사제`로 표시됨.
  - `품질팀`이 `품질`, `팀`으로 분리됨.
  - 많은 값이 `+52`로 접혀 있어 사용자가 핵심 OCR 결과를 바로 대조하기 어려움.

### 원인
- y좌표 클러스터링으로 행은 만들었지만, 원본 표의 고정 열 경계에 맞춰 셀을 재배치하지 못함.
- OCR 토큰이 누락되거나 쪼개졌을 때 빈 셀을 유지하지 않고 남은 토큰을 앞쪽 칸으로 붙여서 행/열이 밀림.
- 후처리는 문자 오인식 일부만 보정하고, `색도 (CIE x)`, `파티클 수 (≥0.5μm)`, `ea/m²` 같은 복합 항목/단위 재조립까지는 처리하지 못함.

### 고쳐야 할 점
- 이미지 OCR 표는 원본 표의 열 경계 기준으로 재구성해야 함.
  - `No.`, `검사항목`, `단위`, `규격(하한)`, `규격(상한)`, `측정치`, `판정` 열이 원본처럼 고정되어야 함.
- OCR이 못 읽은 셀은 조용히 당겨 붙이면 안 됨.
  - 빈 셀로 남기거나 `누락` 상태를 표시해 사용자가 어느 값이 빠졌는지 알 수 있어야 함.
- 품질 검사 샘플 이미지 기준으로 최소 아래 행들은 PDF 텍스트 파싱 결과와 동일하게 표시되어야 함.
  - `1 박막두께 nm 95.0 105.0 101.2 합격`
  - `2 면저항 Ω/sq 8.0 12.0 9.8 합격`
  - `4 접착력 N/cm 5.0 - 7.2 합격`
  - `8 파티클 수 (≥0.5μm) ea/m² 0 50 12 합격`
- OCR 상단 요약은 원본과 다른 오인식 값이 그대로 노출되지 않도록 보정하거나, 표 검수 화면에서는 원본 표 대조에 방해되지 않게 줄여야 함.

코덱스 검수완료

---

## 클로드 8차 재수정 (8차 검수 피드백 반영)

### 수정 항목

1. **OCR 표 구조화 알고리즘 전면 재작성**
   - 기존: 단순 y클러스터 + x정렬 → 14열 (열이 과분할)
   - 변경: 2단계 접근 — ①y클러스터링(행) → ②행 내 근접 워드 합치기(gap≤15px) → ③큰 gap(>40px)으로 열 분리
   - 결과: `파티클 수 (≥0.5μm)` 같은 복합 항목이 한 셀로 합쳐짐

2. **OCR 후처리 보정 확대**
   - `ea/m2→ea/m²`, `검사제→검사자:`, `0/0→%` 추가
   - `u m→μm` 공백 포함 패턴 추가
